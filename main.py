...
class RenomeadorApp:
    ...
    def criar_interface(self):
        ...
        ttk.Label(frame_topo, text="Pasta:").grid(row=0, column=0, sticky="w")

        self.entry_pasta = ttk.Entry(frame_topo, textvariable=self.pasta)
        self.entry_pasta.grid(row=1, column=0, sticky="ew", padx=(0, 5))

        frame_topo.columnconfigure(0, weight=1)

        self.bt_selecionar = ttk.Button(
            frame_topo, text="Selecionar...", command=self.selecionar_pasta
        ).grid(row=1, column=1)

        self.bt_iniciar = ttk.Button(self.root, text="Iniciar", command=self.iniciar)
        self.bt_iniciar.pack(pady=(0, 10))

        ttk.Label(self.root, text="Log").pack(anchor="w", padx=10)

        self.log = ScrolledText(self.root, height=18, state="disabled")
    ...
    def escrever_log(self, texto, substituir=False):
        self.root.after_idle(self._escrever_log, texto, substituir)
    ...
    def habilitar_botao(self, habilitado=True):
        self.root.after(
            0,
            lambda: self.bt_iniciar.config(state="normal" if habilitado else "disabled")
        )

        self.root.after(
            0,
            lambda: self.bt_selecionar.config(state="normal" if habilitado else "disabled")
        )
    ...
    def iniciar(self):
        ...
        thread = threading.Thread(target=self.executar, daemon=True)
        thread.start()
